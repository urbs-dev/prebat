...
<section in:fade|global={{ duration: 300 }}>
    ...
    <ul>
        ...
        <li>
            <div>Organisme : </div>
            <span>{user.organization ?? 'non renseigné'}</span>
        </li>
        <li>
            <div>Fonction : </div>
        ...
        <li>
            <div>Inscription : </div>
            <span>{DateTime.relative(user.created_at)}</span>
        </li>
    </ul>
...
<style>
    ...
    article i{margin:0 10px 0 0;width:80px;height:90px;color:var(--primary);text-align:center;font-size:60px;line-height:90px;background:var(--primary-lighten-1);border-radius:4px;}
    ...
    ul{margin:16px 8px;}
    li{display:flex;justify-content:flex-start;align-items:center;min-height:35px;border-bottom:1px solid #eee;color:#757575;font-size:13px;}
    li div{min-width:80px;white-space:nowrap;}
    li span{color:#424242;word-wrap: break-word;}
    button{justify-content:flex-start;color:var(--primary);margin:8px auto;width:100%;border-radius:4px;font-size:14px;min-height:40px;padding:0 16px;background:#fafafa;}
